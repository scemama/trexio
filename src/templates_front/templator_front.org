...
*** C
   ...
   #+begin_src c :tangle prefix_front.h :exports none
trexio_exit_code trexio_rm(const char* path, back_end_t back_end);
//trexio_exit_code trexio_cp(const char* path, back_end_t back_end);
...
trexio_exit_code trexio_copy_directory_recursive(const char* source, const char* dest);
   ...
   #+begin_src c :tangle prefix_front.c
trexio_exit_code
trexio_rm(const char* path, back_end_t back_end)
{
  switch (back_end) {
  case TREXIO_TEXT:
